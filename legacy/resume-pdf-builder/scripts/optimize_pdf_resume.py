...
def interactive_mode():
    ...
    if not config.openai_api_key:
        error_message("请先配置 OPENAI_API_KEY：")
        console.print("   [cyan]export OPENAI_API_KEY=\"your_api_key\"[/cyan]")
        sys.exit(1)
    ...
    return resume_path, jd_path, output_name


def process_resume_workflow(pdf_resume_path: str, jd_path: str, output_name: str = "optimized_resume.pdf"):
    """
    简历优化的完整工作流程。
...
def main():
    ...
    if args.no_interactive:
        ...
        if not config.openai_api_key:
            error_message("请配置 OPENAI_API_KEY")
            sys.exit(1)

        welcome_banner()
        output_path, suggestions = process_resume_workflow(
            args.resume_pdf, args.jd, args.output
        )

    else:
        # 交互模式
        if not args.resume_pdf or not args.jd:
            resume_path, jd_path, output_name = interactive_mode()
            output_path, suggestions = process_resume_workflow(
                resume_path, jd_path, output_name
            )
        else:
            # 有参数，请求确认
            display_results("确认信息", {
